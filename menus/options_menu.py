...
class OptionsApp(SoftApp):
    ...
    def _build_main_menu(self):
        ...
        kb.add_child(MenuNode("Typing Echo Mode", self._enter_typing_echo_menu))
    ...
    def _load_flat_settings(self):
        flat = {}
        flat["rate"] = settings_manager.get("tts", "rate")
        flat["volume"] = settings_manager.get("tts", "volume")
        flat["voice_index"] = settings_manager.get("tts", "voice_index")
        flat["punctuation_level"] = settings_manager.get("tts", "punctuation_level", "Some")
        flat["pitch"] = settings_manager.get("tts", "pitch", 50)
        flat["capital_pitch_change"] = settings_manager.get("tts", "capital_pitch_change", "Off")
        flat["volume_ducking"] = settings_manager.get("audio", "volume_ducking", "Off")
        flat["sound_scheme"] = settings_manager.get("audio", "sound_scheme", "Default")
        flat["typing_echo_mode"] = settings_manager.get("keyboard", "typing_echo_mode", "")
        flat["char_echo"] = settings_manager.get("keyboard", "char_echo", "Off")
        flat["word_echo"] = settings_manager.get("keyboard", "word_echo", "Off")
        flat["announce_position"] = settings_manager.get("keyboard", "announce_position", "On")
        flat["state_keys"] = settings_manager.get("keyboard", "state_keys", "Off")
        flat["speech_history_size"] = settings_manager.get("tts", "speech_history_size", 50)
        flat["update_channel"] = settings_manager.get("system", "update_channel", "stable")
        flat["auto_update_on_startup"] = settings_manager.get("system", "auto_update_on_startup", False)
        flat["voice_profiles"] = settings_manager.get("custom", "voice_profiles", {})
        flat["per_app_voice"] = settings_manager.get("custom", "per_app_voice", {})
        return flat

    def _load_voice_settings(self):
        try:
            loaded = self._load_flat_settings()
            self.settings.clear()
            self.settings.update(loaded)
            rate = self.settings.get("rate")
            if rate is not None:
                self.manager.synth.set_rate(rate)
            volume = self.settings.get("volume")
            if volume is not None:
                self.manager.synth.set_volume(volume)
            voice_index = self.settings.get("voice_index")
            if voice_index is not None:
                names = self.manager.synth.get_voice_names()
                if 0 <= voice_index < len(names):
                    self.manager.synth.set_voice_by_index(voice_index)
            pl = self.settings.get("punctuation_level", "Some")
            self.manager.synth.set_punctuation_level(pl)
            pitch = self.settings.get("pitch", 50)
            self.manager.synth.set_pitch(pitch)
            cpc = self.settings.get("capital_pitch_change", "Off")
            self.manager.synth.set_capital_pitch_change(cpc)
            vd = self.settings.get("volume_ducking", "Off") == "On"
            self.manager.synth.set_volume_ducking(vd)
            import core.menu
    ...
    def _save_settings(self):
        try:
            ...
            tts = settings_manager.get_category("tts")
            tts.update({
                "rate": self.settings.get("rate"),
                "volume": self.settings.get("volume"),
                "voice_index": self.settings.get("voice_index"),
                "punctuation_level": self.settings.get("punctuation_level", "Some"),
                "pitch": self.settings.get("pitch", 50),
                "capital_pitch_change": self.settings.get("capital_pitch_change", "Off"),
                "speech_history_size": self.settings.get("speech_history_size", 50),
            })
            settings_manager.set_multi("tts", tts)
            tem = self.settings.get("typing_echo_mode", "")
            ce = self.settings.get("char_echo", "Off")
            we = self.settings.get("word_echo", "Off")
            if tem == "char":
                ce, we = "On", "Off"
            elif tem == "word":
                ce, we = "Off", "On"
                ...
                ce, we = "Off", "Off"
            settings_manager.set("keyboard", "typing_echo_mode", tem)
            settings_manager.set("keyboard", "char_echo", ce)
            settings_manager.set("keyboard", "word_echo", we)
            settings_manager.set("keyboard", "announce_position", self.settings.get("announce_position", "On"))
            settings_manager.set("keyboard", "state_keys", self.settings.get("state_keys", "Off"))
            settings_manager.set("audio", "volume_ducking", self.settings.get("volume_ducking", "Off"))
            settings_manager.set("audio", "sound_scheme", self.settings.get("sound_scheme", "Default"))
            settings_manager.set("system", "update_channel", self.settings.get("update_channel", "stable"))
            settings_manager.set("system", "auto_update_on_startup", self.settings.get("auto_update_on_startup", False))
            settings_manager.set("custom", "voice_profiles", self.settings.get("voice_profiles", {}))
            settings_manager.set("custom", "per_app_voice", self.settings.get("per_app_voice", {}))
    ...
    def _enter_typing_echo_menu(self):
        self._current_parent_back = self._back_to_kb_menu
        self._build_list_menu("Typing Echo Mode", "typing_echo_mode",
                             ["off", "char", "word"],
                             side_effect=self._typing_echo_side_effect)

    def _typing_echo_side_effect(self, key, value):
        if value == "char":
            self.settings["char_echo"] = "On"
            self.settings["word_echo"] = "Off"
        elif value == "word":
            self.settings["char_echo"] = "Off"
            self.settings["word_echo"] = "On"
        elif value == "off":
            self.settings["char_echo"] = "Off"
            self.settings["word_echo"] = "Off"
    ...
    def _enter_sound_scheme_menu(self):
        ...
        self._build_list_menu("Sound Scheme", "sound_scheme",
                             ["Default", "Minimal", "Classic", "Modern", "Soft", "Loud", "Natural", "None"],
    ...
    def _load_bindings(self):
        return dict(settings_manager.get("custom", "key_bindings", {}))

    def _save_bindings(self, bindings):
        settings_manager.set("custom", "key_bindings", bindings)
        settings_manager.save()
    ...
    def _enter_plugin_manager(self):
        ...
        root.add_child(MenuNode("Browse Plugin Store", self._pm_browse_store))
        root.add_child(MenuNode("Installed Plugins", self._pm_show_installed))
        root.add_child(MenuNode("Install from File...", self._pm_install_file))
        root.add_child(MenuNode("Refresh Catalog", self._pm_refresh_catalog))
        root.add_child(MenuNode("Back", self._current_parent_back))
        self._switch_to_submenu(root, "Plugin Manager")

    def _pm_back_to_plugin(self):
        self._pm_confirm_name = None
        self._current_parent_back = self._back_to_main_menu
        root = MenuNode("Plugin Manager")
        root.add_child(MenuNode("Browse Plugin Store", self._pm_browse_store))
    ...
    def _pm_refresh_catalog(self):
        ...
        try:
            ...
            self._pm_catalog = self._pm.get_catalog()
        except Exception as e:
            self.speak(f"Failed: {e}")
        self._pm_back_to_plugin()

    def _pm_browse_store(self):
        if not self._pm_catalog:
            result = self._pm.fetch_catalog()
            if not result:
                self._pm_catalog = self._pm.get_catalog()
            else:
                self._pm_catalog = result
        if not self._pm_catalog:
            self.speak("No plugin catalog available. Try Refresh Catalog.")
            self._pm_back_to_plugin()
            return
        root = MenuNode("Plugin Store")
        for p in sorted(self._pm_catalog, key=lambda x: x.get('name', '').lower()):
            ...
            if author:
                label += f" by {author}"
            root.add_child(MenuNode(label, lambda info=p: self._pm_catalog_detail(info)))
        root.add_child(MenuNode("Back", self._pm_back_to_plugin))
        self._switch_to_submenu(root, "Plugin Store")

    def _pm_catalog_detail(self, info):
        ...
        if existing:
            ...
            root.add_child(MenuNode("Install", lambda: self._pm_install_from_catalog(info)))
        root.add_child(MenuNode("Back", self._pm_browse_store))
        self._switch_to_submenu(root, name)

    def _pm_install_from_catalog(self, info):
        name = info.get('name', 'Unknown')
        ...
        self._pm_browse_store()

    def _pm_show_installed(self):
        ...
        if not plugins:
            ...
            for p in sorted(plugins, key=lambda x: x['name'].lower()):
                ...
                root.add_child(MenuNode(label, lambda info=p: self._pm_installed_detail(info)))
        root.add_child(MenuNode("Back", self._pm_back_to_plugin))
        self._switch_to_submenu(root, "Installed Plugins")

    def _pm_installed_detail(self, info):
        ...
        root.add_child(MenuNode("Reload", lambda: self._pm_reload(info['name'])))
        root.add_child(MenuNode("Uninstall", lambda: self._pm_confirm_uninstall(info['name'])))
        root.add_child(MenuNode("Back", self._pm_show_installed))
        self._switch_to_submenu(root, info['name'])

    def _pm_reload(self, name):
        try:
            self._pm.reload_plugin(name)
            self.speak(f"{name} reloaded.")
        except Exception as e:
            self.speak(f"Reload failed: {e}")
        self._pm_show_installed()

    def _pm_confirm_uninstall(self, name):
        root = MenuNode("Uninstall Plugin")
        root.add_child(MenuNode(f"Uninstall {name}?", lambda: self._pm_do_uninstall(name)))
        root.add_child(MenuNode("Cancel", self._pm_show_installed))
        self._switch_to_submenu(root, f"Uninstall {name}?")

    def _pm_do_uninstall(self, name):
        try:
    ...
    def _pm_install_file(self):
        path = self._pm_pick_file()
        if path:
            try:
